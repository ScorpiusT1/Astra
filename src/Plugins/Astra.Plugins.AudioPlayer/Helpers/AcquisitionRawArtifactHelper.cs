...
{
    /// <summary>
    /// 按采集卡显示名解析上游 Raw 工件键。BFS 逻辑委托给 <see cref="RawDataPipelineResolver"/>。
    /// </summary>
    internal static class AcquisitionRawArtifactHelper
    {
            ...
            out string error)
        {
            ...
            if (!RawDataPipelineResolver.TryFindUpstreamRawProducerNodeId(wf, playbackNodeId, deviceName, out var producerNodeId))
            {
                error = $"上游未找到采集卡「{deviceName}」对应的 Raw（多采集、数字滤波或文件导入），请将本节点接在相应节点之后";
                return false;
            }
