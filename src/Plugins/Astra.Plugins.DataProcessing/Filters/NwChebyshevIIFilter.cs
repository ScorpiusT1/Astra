...
using NWaves.Filters.ChebyshevII;

namespace Astra.Plugins.DataProcessing.Filters
{
    /// <summary>NWaves Chebyshev II（阻带等纹波）。</summary>
    internal static class NwChebyshevIIFilter
    {
            ...
            double rippleDb)
        {
            ...
            if (rippleDb <= 0)
                throw new ArgumentOutOfRangeException(nameof(rippleDb), "阻带纹波须为正数 (dB)。");
