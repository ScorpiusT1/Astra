...
using NWaves.Filters.Elliptic;

namespace Astra.Plugins.DataProcessing.Filters
{
    /// <summary>NWaves 椭圆（Cauer）滤波器，最窄过渡带。</summary>
    internal static class NwEllipticFilter
    {
            ...
            double rippleStopDb)
        {
            ...
            if (ripplePassDb <= 0)
                throw new ArgumentOutOfRangeException(nameof(ripplePassDb), "通带纹波须为正数 (dB)。");
            if (rippleStopDb <= 0)
                throw new ArgumentOutOfRangeException(nameof(rippleStopDb), "阻带衰减须为正数 (dB)。");
            ...
            switch (kind)
            {
                case ButterworthFilterKind.LowPass:
                    NwIirFilterValidation.ValidateHz(cutoffOrLowHz, ny);
                    {
                        ...
                        var filter = new LowPassFilter(f, order, ripplePassDb, rippleStopDb);
                ...
                case ButterworthFilterKind.HighPass:
                    ...
                    {
                        var f = cutoffOrLowHz / samplingRate;
                        var filter = new HighPassFilter(f, order, ripplePassDb, rippleStopDb);
                        return NwSignalFilterUtil.ToDoubleArray(filter.ApplyTo(input));
                    }
                case ButterworthFilterKind.BandPass:
                        ...
                        throw new ArgumentException("带通低频须小于高频。");
                    {
                        ...
                        var filter = new BandPassFilter(f1, f2, order, ripplePassDb, rippleStopDb);
                        return NwSignalFilterUtil.ToDoubleArray(filter.ApplyTo(input));
                    }
                default:
