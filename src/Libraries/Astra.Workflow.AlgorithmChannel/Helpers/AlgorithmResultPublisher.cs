...
namespace Astra.Workflow.AlgorithmChannel.Helpers
{
    ...
    /// </summary>
    public static class AlgorithmResultPublisher
    {
        public static ExecutionResult SuccessWithChart(
            ...
            string? tag = null,
            string message = "完成",
            Dictionary<string, object>? artifactPreview = null,
            bool includeInTestReport = true)
        {
            ...
            var r = bus.PublishAlgorithmResult(producerNodeId, artifactName, chart, tag: tag, parameters: artifactPreview, includeInTestReport: includeInTestReport);
            ...
            bool includeInTestReport = true)
        {
            ...
            if (charts.Count == 1)
                return SuccessWithChart(context, producerNodeId, artifactName, charts[0].Chart, tag, message, artifactPreview, includeInTestReport);
            ...
            return SuccessWithChart(context, producerNodeId, artifactName, merged, tag, message, artifactPreview, includeInTestReport);
        }
            ...
            bool includeInTestReport = true)
        {
            var chartForBus = scalars is { Count: > 0 }
                ...
                : chart;
            var result = SuccessWithChart(context, producerNodeId, artifactName, chartForBus, tag, message, artifactPreview, includeInTestReport);
            var bus = context.GetDataBus();
            if (bus == null)
                return result;
            ...
            {
                bus.PublishScalar(producerNodeId, name, value, unit: unit, tag: tag ?? artifactName, includeInTestReport: includeInTestReport);
            }
            ...
            bool includeInTestReport = true)
        {
            ...
            var result = SuccessWithMultiChart(context, producerNodeId, artifactName, chartsForBus, layoutOverride, tag, message, artifactPreview, includeInTestReport);
            var bus = context.GetDataBus();
            if (bus == null)
                return result;
            ...
            bool includeInTestReport = true)
        {
            ...
            foreach (var (name, value, unit) in scalars)
                bus.PublishScalar(producerNodeId, name, value, unit: unit, tag: tag, includeInTestReport: includeInTestReport);
